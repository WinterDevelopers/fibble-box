...
def ticket_payment_verification(request, token):
    ...
    if verified:
        ...
        messages.success(
            request, "your ticket was sucessfully purchased and sent to your E-mail"
        )
        ...
        messages.error(
            request, "ticket purchasing failed due to unverified transaction"
        )
...
def shipping_process(request):
    ...
    shippingDetails.objects.create(
        name=name, email=email, phone=phone, order=order, amount=amount, token=token
    )
...
def reference(request):
    ...
    shipping = get_object_or_404(
        shippingDetails, order=order, verification_status=False
    )
    ...
    return JsonResponse(
        {"reference": reference, "token": token}, safe=False, status=200
    )
